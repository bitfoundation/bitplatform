...
*   For anything related to `Bit.BlazorUI`, `bit Bswup`, `bit Butil`, `bit Besql`, or the bit project template, use the `DeepWiki_ask_question` tool with repository `bitfoundation/bitplatform`. For component discovery and API details, also use the `GetBitBlazorUIComponentsList` and `GetComponentParameters` MCP tools.
