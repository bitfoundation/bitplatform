...
Agent Skills are canonical in `.github/agents/` (see `AGENTS.md` section 7 for the list). Claude Code only discovers skills under `.claude/skills/`, so that folder holds one thin bridge stub per skill which delegates to the canonical file. When changing a skill, edit the file in `.github/agents/` and keep only the frontmatter of the matching stub in sync - never fork the body.
