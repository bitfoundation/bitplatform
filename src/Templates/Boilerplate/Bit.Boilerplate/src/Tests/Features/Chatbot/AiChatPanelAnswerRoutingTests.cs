...
/// <summary>
/// An answer is one json document (See <see cref="AssistantReply"/>), streamed down the hub method the panel is
...
public partial class AiChatPanelAnswerRoutingTests : AiChatPanelTestBase
{
    ...
    public async Task Panel_Should_ShowTheSuggestionsTheModelOffered_AndSendTheOneTapped()
    {
        ...
        await Expect(panel).Not.ToContainTextAsync("{\"answer\"");
    ...
    /// <summary>
    /// What the schema makes a real model write: one json document with the answer, split into pieces that are not
    ...
    private static ChatResponseUpdate[] AnswerAsOneDocument(int callIndex, ChatMessage[] conversation)
    {
        ...
        // Seven characters at a time, so the splits land mid string and mid property name.
