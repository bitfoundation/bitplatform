...
public class ChatbotVoiceCallTests
{
    ...
    public async Task StartVoiceCall_Should_OpenTheCallWithTheServersInstructionsAndTools()
    {
        ...
        Assert.Contains("[[[INSTRUCTIONS_BEGIN]]]", instructions!, "The call must get the tenant's system prompt.");
    ...
    /// <summary>Only a response that had already spoken, and did nothing but show suggestions, goes without a new one.</summary>
    [TestMethod]
    [DataRow(false, false, DisplayName = "Nothing was said yet")]
    [DataRow(true, true, DisplayName = "Another tool ran beside them")]
    public async Task SuggestionsThatAreNotAllTheResponseNeeds_Should_StillStartAResponse(bool spoke, bool anotherTool)
    {
        var realtime = new TestRealtimeCallClient();
        ...
        if (spoke)
        {
            output.Insert(0, Spoken("Here you go."));
        }
    ...
    private async Task<AppTestServer> StartServer(TestRealtimeCallClient realtime)
    {
    ...
    /// <summary>What the model said in a response, as the provider reports it.</summary>
    private static JsonObject Spoken(string transcript) => new()
    {
        ["type"] = "message",
        ["role"] = "assistant",
