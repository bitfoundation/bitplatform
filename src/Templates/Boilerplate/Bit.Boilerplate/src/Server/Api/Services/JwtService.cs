...
public partial class JwtService : IJwtService
{
    ...
    public async Task<SignInResponseDto> GenerateToken(User user)
    {
        ...
        RSA? rsaPrivateKey;
        using (X509Certificate2 signingCert = new X509Certificate2(certificatePath, _AppSettings.IdentitySettings.IdentityCertificatePassword, OperatingSystem.IsWindows() ? X509KeyStorageFlags.EphemeralKeySet : X509KeyStorageFlags.DefaultKeySet))
        {
