...
public static partial class Program
{
    ...
    private static void Configure_401_403_404_Pages(WebApplication app)
    {
        app.Use(async (context, next) =>
        {
            ...
            {
                if (context.Request.Path.Value.Contains(PageUrls.NotFoundPage, StringComparison.InvariantCultureIgnoreCase))
                {
                    context.Response.StatusCode = (int)HttpStatusCode.NotFound;
                }
                if (context.Request.Path.Value.Contains(PageUrls.NotAuthorizedPage, StringComparison.InvariantCultureIgnoreCase))
                {
        ...
        app.UseStatusCodePages(options: new()
        {
            HandleAsync = async (statusCodeContext) =>
            {
                    ...
                    httpContext.GetEndpoint()?.Metadata.OfType<ComponentTypeMetadata>().Any() is true /* The generation of a 401 or 403 status code is attributed to Blazor. */)
                {
                    ...
                    httpContext.Response.Redirect($"{PageUrls.NotAuthorizedPage}?return-url={returnUrl}&isForbidden={(is403 ? "true" : "false")}");
                }
                ...
                {
                    httpContext.Response.Redirect($"{PageUrls.NotFoundPage}?url={httpContext.Request.GetEncodedPathAndQuery()}");
                }
                else
                {
