...
public static class WebApplicationBuilderExtensions
{
        ...
        where TBuilder : IHostApplicationBuilder
    {
        ...
        builder.AddRedisClient("redis-persistent", settings => settings.DisableTracing = true, options => options.Ssl = builder.Environment.IsDevelopment() is false);

        // Add optional Redis connection for caching (ephemeral Redis without persistence)
        builder.AddKeyedRedisClient("redis-cache", settings => settings.DisableTracing = true /*FusionCache is already handling cache traces*/, options => options.Ssl = builder.Environment.IsDevelopment() is false);
        //#endif
