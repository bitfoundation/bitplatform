...
/// instructions and tools, runs the tools the model calls, and hangs up on time limit or when the user starts another.
/// <para>
    ...
    ILogger<VoiceCallRunner> logger)
{
    ...
    public async Task<string> Start(VoiceCallStart start, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            chatbot.UseSignalRConnection(start.SignalRConnectionId);
            ...
            var lifetime = new CancellationTokenSource(MaxCallDuration);
    ...
    private async Task Run(AsyncServiceScope scope, IRealtimeSideband sideband, List<AIFunction> functions, string callId, VoiceCallStart start, CancellationTokenSource lifetime)
    {
            ...
            await foreach (var serverEvent in sideband.ReadEvents(lifetime.Token))
            {
                switch ((string?)serverEvent["type"])
                {
                    case "response.done":
                        ...
                        await AnswerToolCalls(serverEvent, sideband, functions, lifetime.Token);
                        break;
        ...
        {
            // Time limit, a newer call of the same user, or the provider ended it.
        }
        catch (Exception exp)
        {
    ...
    private async Task AnswerToolCalls(JsonObject responseDone, IRealtimeSideband sideband, List<AIFunction> functions, CancellationToken cancellationToken)
    {
        ...
        // Already spoken and only showed suggestions: a new response would talk about them.
        if (items.Any(item => (string?)item["type"] is "message") && calls.All(call => (string?)call["name"] is "ShowFollowUpSuggestions")) return;
