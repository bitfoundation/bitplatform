...
- a `### Voice call:` section that turns off markdown and links, since nothing spoken is shown as text, and says an approval takes a tap on the screen
- the variables, which come from the client and are therefore cleaned first: `SanitizeVariable` for the device, `KnownTimeZoneId` for the time zone.

The controller also hashes the user id into the `OpenAI-Safety-Identifier`.

**The runner creates the call.** It opens a DI scope that lives for the whole call and takes `GetAIFunctions()` from its `AppChatbot`, the same tools the text chat uses. `POST /realtime/calls` sends the offer together with the whole `session`: model, instructions, voice, reasoning effort (when set), transcription model and language (the user's culture), `tool_choice: auto` and each tool's JSON schema. OpenAI returns the SDP answer in the body and the call id in the `Location` header.
...
**The runner starts the call's lifetime.** A `CancellationTokenSource` ends the call after `MaxCallDuration`, or when the app stops. Storing it in `callsByUser` cancels the user's previous call, because every open call is billed by the minute. The event loop then runs on its own, detached from the HTTP request.
...
4. Tools that act on the running app (`NavigateToPage`, `SetApplicationTheme`, `SetApplicationCulture`, `CheckLastError`, `ClearAppFiles`, `ShowSignInModal`, and the ones showing cards or suggestions) use SignalR to reach the tab. Before each round of calls, the runner reads the session's current connection id, so a tab that reconnected, even to another instance, still gets them. `ClearAppFiles` waits there for the user to tap its approval card, and the call's events wait with it, for at most the call's remaining time. Tools that stay on the server skip this step.
...
The runner's loop also ends when `MaxCallDuration` is reached, when the same user starts a newer call, or when the app stops, though the app doesn't wait for that hang-up. In every case, the `finally` block does the following:
...
| `RealtimeReasoningEffort` | Sent in `session` as `reasoning.effort`: `minimal`, `low`, `medium`, `high` or `xhigh`. Leave it empty for the model's default, `low` for gpt-realtime-2 and 2.1. Higher effort adds latency and output tokens. |
