...
public partial class IdentityController
{
    ...
    private async Task SendConfirmEmailToken(User user, string? returnUrl, CancellationToken cancellationToken)
    {
        ...
        var link = new Uri(HttpContext.Request.GetWebAppUrl(), $"{Urls.ConfirmPage}?email={UriUtils.Escape(email)}&emailToken={UriUtils.Escape(token)}&culture={CultureInfo.CurrentUICulture.Name}&return-url={UriUtils.Escape(returnUrl)}");
