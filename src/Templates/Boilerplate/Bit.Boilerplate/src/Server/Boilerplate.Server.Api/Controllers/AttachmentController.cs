...
public partial class AttachmentController : AppControllerBase, IAttachmentController
{
    ...
    private async Task PublishUserProfileUpdated(User user, CancellationToken cancellationToken)
    {
        ...
        await appHubContext.Clients.Clients(userSessionIdsExceptCurrentUserSessionId).SendAsync(SharedPubSubMessages.PUBLISH_MESSAGE, SharedPubSubMessages.PROFILE_UPDATED, user.Map(), cancellationToken);
    }
    //#endif
