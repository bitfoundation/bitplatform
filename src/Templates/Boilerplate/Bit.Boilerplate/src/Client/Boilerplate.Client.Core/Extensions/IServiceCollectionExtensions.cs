...
public static class IServiceCollectionExtensions
{
    public static IServiceCollection AddClientCoreProjectServices(this IServiceCollection services)
    {
        ...
        services.AddBesqlDbContextFactory<OfflineDbContext>(options =>
        {
            ...
            var dirPath = isRunningInsideDocker ? "/container_volume"
                                                : AppOperatingSystem.IsBlazorHybrid || OperatingSystem.IsBrowser() ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AC87AA5B-4B37-4E52-8468-2D5DF24AF256")
    ...
    {
        if (AppOperatingSystem.IsBlazorHybrid || OperatingSystem.IsBrowser())
        {
