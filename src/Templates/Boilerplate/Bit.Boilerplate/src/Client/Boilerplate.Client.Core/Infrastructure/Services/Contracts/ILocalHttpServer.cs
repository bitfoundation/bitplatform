...
public interface ILocalHttpServer : IAsyncDisposable
{
    ...
    /// <summary>
    /// Random per-process token that every request to the local server must carry. The server is bound to
    /// loopback with no authentication of its own, and a POST with no custom headers is a CORS "simple request",
