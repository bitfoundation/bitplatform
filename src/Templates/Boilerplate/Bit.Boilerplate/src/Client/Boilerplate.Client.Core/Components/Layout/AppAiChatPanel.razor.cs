...
public partial class AppAiChatPanel : IAiChatCardHost
{
    ...
    /// <summary>The card the server waits on for the user's answer. One at a time: a newer one expires it.</summary>
    private (AiChatCard Card, TaskCompletionSource<string> Decision)? awaitedCard;
    ...
    /// <summary>The server's wait on a card ends with the connection it asked over (See AppChatbot.AwaitCard).</summary>
    private Task HubConnection_Reconnecting(Exception? _) => InvokeAsync(ExpireAwaitedCard);
    ...
    /// <summary>Appends a card the server sent, and stores it unless it stores itself. One this app version has no component for shows its RawMarkdown.</summary>
    private async Task<bool> ShowCard(AiChatCard card)
    {
        await InvokeAsync(async () =>
        {
            ...
            if (ResolveCardComponent(card.ComponentType)?.IsAssignableTo(typeof(IStoresItself)) is not true)
            {
    ...
    private async Task<string> AwaitCard(AiChatCard card)
    {
        ...
        await InvokeAsync(async () =>
        {
            await ExpireAwaitedCard();

            awaitedCard = (card, decision);
            AddCard(card);
        });
    ...
    /// <summary>Above the answer still streaming, which talks about the card: the order the server remembers them in too.</summary>
    private void AddCard(AiChatCard card)
    {
        var pendingAnswer = lastAssistantMessage is not null && unansweredMessages.Contains(lastAssistantMessage)
    ...
    /// <summary>Answers the card the server waits on, once, and stores it with that answer.</summary>
    private async Task ResolveCard(AiChatCard card, string decision)
    {
        if (awaitedCard is not { } awaited || ReferenceEquals(awaited.Card, card) is false) return;

        awaitedCard = null;

        card.Data["Decision"] = decision;

        awaited.Decision.TrySetResult(decision);
    ...
    /// <summary>Ends the wait the server gave up on: after a new message, a newer card, a clear, a hang-up or a lost connection.</summary>
    private Task ExpireAwaitedCard() => awaitedCard is { } awaited ? ResolveCard(awaited.Card, AiChatCardDecision.NoAnswer) : Task.CompletedTask;
    ...
    bool IAiChatCardHost.IsAwaited(AiChatCard card) => ReferenceEquals(awaitedCard?.Card, card);
    ...
    private async Task ReadTurnSoFar(string chunk)
    {
        ...
        {
            answer.Content = turn?.Answer ?? answer.Content;
            return;
        }
        ...
        {
            // What streamed is kept and tagged, but not re-read: a cut-off turn has its closing spliced onto half an
            // answer.
