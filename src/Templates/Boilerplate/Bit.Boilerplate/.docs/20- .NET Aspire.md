...
- **Built-in Development Tools**: 
  ...
  - **RustFS** for S3-compatible storage testing
...
**Call `builder.UsePersistentContainers()` unconditionally** - see the next section. That keeps the database (and every other
...
```json
{
    "Parameters": {
        ...
        "rustfs-accessKey": "rustfsadmin",
        "rustfs-secretKey": "P@ssw0rd",
...
`builder.UsePersistentContainers()` at the bottom of `Program.cs` in the `Boilerplate.Server.AppHost` project keeps the containers themselves alive and reuses them instead. Out of the box it only runs in environments that exist just for this project - a Dev Container, GitHub Codespaces, a GitHub Actions runner or an Azure DevOps pipeline agent (See `IsDedicatedEnvironment` in `IDistributedApplicationBuilderExtensions.cs`) - where it also speeds up CI jobs that run the tests several times:
...
To have it on your own machine as well, remove the `if` and keep a bare `builder.UsePersistentContainers();`.
...
**If this is your main project and you work on it every day, removing the `if` is recommended.**
...
```xml
<PackageReference Include="AspireIntegration.Hosting.RustFs" />  <!-- filesStorage == S3 -->
```
