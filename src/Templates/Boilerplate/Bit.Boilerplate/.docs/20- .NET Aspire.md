...
1. **One extension method per resource**: each wraps an `AddAzure…(...).RunAsContainer(...)` pair, so the same code
   runs a container locally and provisions the real Azure resource when you deploy - remove the `RunAsContainer` to
   switch
...
By default every container (database, Keycloak, Redis, MailPit, storage and their management UIs) is created from scratch on each run and destroyed when the app host stops. There's a ready-made line at the bottom of `Program.cs` in the `Boilerplate.Server.AppHost` project that keeps them all alive and reuses them instead — just uncomment it:
