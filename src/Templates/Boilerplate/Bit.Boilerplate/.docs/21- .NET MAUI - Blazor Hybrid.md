...
**Location**: [`/src/Client/Boilerplate.Client.Web/wwwroot/.well-known/apple-app-site-association.json`](/src/Client/Boilerplate.Client.Web/wwwroot/.well-known/apple-app-site-association.json)
...
Implements `IStorageService` using MAUI's `Preferences` API in [`MauiStorageService.cs`](/src/Client/Boilerplate.Client.Maui/Infrastructure/Services/MauiStorageService.cs):
...
Implements `IExternalNavigationService` for opening external URLs in [`MauiExternalNavigationService.cs`](/src/Client/Boilerplate.Client.Maui/Infrastructure/Services/MauiExternalNavigationService.cs):
