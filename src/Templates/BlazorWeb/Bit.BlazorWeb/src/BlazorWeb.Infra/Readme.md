...
2- Create bw-prod resource group

```
az group create --name bw-prod --location eastus
```

Notes:
* `bw` is an abbreviation for BlazorWeb, use the acronym of your choice and replace bw with that (for example abc) using exact match - case sensitive find and replace in this file and AdStack.cs.
...
```
az bw sp create-for-rbac -n "bw-prod" --role Contributor --scopes /subscriptions/{subscriptionId}/resourceGroups/bw-prod
```
...
* Running `az bw sp` will return a json like response that contains `appId`l, `password` and `tenant`. Store them somewhere safe.
...
pulumi config set BlazorWeb.Iac:sql-server-bw-db-admin-id
pulumi config set BlazorWeb.Iac:sql-server-bw-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set BlazorWeb.Iac:default-email-from
pulumi config set BlazorWeb.Iac:email-server-host
pulumi config set BlazorWeb.Iac:email-server-port
pulumi config set BlazorWeb.Iac:email-server-userName
pulumi config set BlazorWeb.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set BlazorWeb.Iac:identity-certificate-password --secret
```
