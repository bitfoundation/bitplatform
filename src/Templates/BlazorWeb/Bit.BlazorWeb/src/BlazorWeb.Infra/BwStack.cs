...
public class BpStack : Stack
{
    public BpStack()
    {
        ...
        var sqlDatabaseDbAdminId = pulumiConfig.Require("sql-server-bw-db-admin-id");
        var sqlDatabaseDbAdminPassword = pulumiConfig.RequireSecret("sql-server-bw-db-admin-password");
        ...
        ResourceGroup resourceGroup = new($"bw-{stackName}", new ResourceGroupArgs
        {
            ResourceGroupName = $"bw-{stackName}"
        }, options: new() { ImportId = $"/subscriptions/{GetClientConfig.InvokeAsync().GetAwaiter().GetResult().SubscriptionId}/resourceGroups/bw-prod" });

        Workspace appInsightsWorkspace = new($"insights-wkspc-bw-{stackName}", new()
        {
            WorkspaceName = $"insights-wkspc-bw-{stackName}",
            ResourceGroupName = resourceGroup.Name,
            Location = resourceGroup.Location,
            RetentionInDays = 30
        });

        AppInsights appInsights = new($"app-insights-bw-{stackName}", new()
        {
            ResourceName = $"app-insights-bw-{stackName}",
            ResourceGroupName = resourceGroup.Name,
            Location = resourceGroup.Location,
        ...
        SqlServer sqlServer = new($"sql-server-bw-{stackName}", new()
        {
            ServerName = $"sql-server-bw-{stackName}",
            ResourceGroupName = resourceGroup.Name,
            Location = resourceGroup.Location,
        ...
        SqlDatabase sqlDatabase = new($"sql-database-bw-{stackName}", new()
        {
            DatabaseName = $"sql-database-bw-{stackName}",
            ResourceGroupName = resourceGroup.Name,
            Location = resourceGroup.Location,
        ...
        AppServicePlan appServicePlan = new($"app-plan-bw-{stackName}", new()
        {
            Name = $"app-plan-bw-{stackName}",
            ResourceGroupName = resourceGroup.Name,
            Location = resourceGroup.Location,
            Kind = "Linux",
            Reserved = true,
        ...
        string vaultName = $"vault-bw-{stackName}";
        ...
        WebApp webApp = new($"app-service-bw-{stackName}", new()
        {
            Name = $"app-service-bw-{stackName}",
            ResourceGroupName = resourceGroup.Name,
            Location = resourceGroup.Location,
            ...
            SiteConfig = new SiteConfigArgs
            {
                ...
                AppCommandLine = "dotnet BlazorWeb.Server.dll",
                AppSettings =
                [
        ...
        Vault vault = new Vault($"vault-bw-{stackName}", new()
        {
            ResourceGroupName = resourceGroup.Name,
            Location = resourceGroup.Location,
            VaultName = vaultName,
        ...
        Secret sqlDatabaseConnectionStringSecret = new(sqlDatabaseConnectionStringSecretName, new()
        {
            ...
            Properties = new SecretPropertiesArgs
            {
                Value = Output.Tuple(sqlServer.Name, sqlDatabase.Name, sqlDatabaseDbAdminPassword).Apply(t =>
                {
                    ...
                    return $"Data Source=tcp:{_sqlServer}.database.windows.net;Initial Catalog={_sqlDatabase};User ID={sqlDatabaseDbAdminId};Password={_sqlDatabasePassword};Application Name=BlazorWeb;Encrypt=True;";
                })
            }
        });
    }
