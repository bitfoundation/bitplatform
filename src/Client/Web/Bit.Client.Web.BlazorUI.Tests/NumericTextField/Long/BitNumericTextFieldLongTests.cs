...
namespace Bit.Client.Web.BlazorUI.Tests.NumericTextField.Long
{
    ...
    public class BitNumericTextFieldLongTests : BunitTestContext
    {
        ...
        {
            var component = RenderComponent<BitNumericTextFieldLongTest>(parameters => parameters.Add(p => p.Suffix, suffix));
        ...
        public void BitNumericTextFieldShouldHaveCorrectMaxMin(long? min, long? max)
        {
            ...
            {
                expectedMinValue = long.MinValue;
                expectedMaxValue = long.MaxValue;
            }
        ...
        public void BitNumericTextFieldTwoWayBoundWithCustomHandlerShouldWorkCurrect(long value, int countOfIncrements, long step)
        {
        ...
        private long? Normalize(long? value, int precision) => value is null ? null : (long?)Math.Round((double)value.Value, precision);
