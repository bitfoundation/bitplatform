...
/// placeholders, sticky group headers, and a bottom-anchored (chat) mode with scroll anchoring that
/// prevents content from jumping as dynamic items get measured.
/// </summary>
public partial class BitVirtualize<TItem> : BitComponentBase
{
    ...
    /// gets pinned to the leading edge of the viewport while its group scrolls. Only supported with in-memory Items.
    /// </summary>
    ...
    public async Task ScrollToIndexAsync(int index, BitVirtualizeScrollAlignment alignment = BitVirtualizeScrollAlignment.Start, bool smooth = false)
    {
        ...
        var maxOffset = Math.Max(0, GetTotalSize() - _viewportSize);
        target = Math.Clamp(target, 0, maxOffset);
        await _js.BitVirtualizeScrollToOffset(UniqueId, target, smooth);
    }
    ...
    public async Task ScrollToOffsetAsync(double offset, bool smooth = false)
    {
        ...
        var maxOffset = Math.Max(0, GetTotalSize() - _viewportSize);
        await _js.BitVirtualizeScrollToOffset(UniqueId, Math.Clamp(offset, 0, maxOffset), smooth);
    }
    ...
    public async Task _ItemsMeasured(int[] indices, double[] sizes)
    {
        ...
        {
            _scrollOffset += diff;
            await _js.BitVirtualizeAdjustScroll(UniqueId, diff);
        }
    ...
    private async Task ApplyPendingScrollAsync()
    {
        ...
        else if (_preserveEndDistance >= 0)
        {
            ...
            await _js.BitVirtualizeScrollToOffset(UniqueId, target, false);
        }
    }
    ...
    private async Task LoadProviderWindowAsync(bool forceCount)
    {
        ...
        if (forceCount is false && _loadedItems is not null &&
    ...
    private void RecomputeRange()
    {
        ...
        var end = FindIndexAtOffset(offset + _viewportSize) + 1;
    ...
    private void UpdateSticky()
    {
        ...
        _stickyNextOffset = next >= 0 ? GetItemOffset(next) : -1;
    }
