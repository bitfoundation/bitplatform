...
public partial class BitQuickGrid<TGridItem> : IAsyncDisposable
{
    ...
    private List<BitQuickGridColumnBase<TGridItem>> _columns;
    ...
    private BitQuickGridColumnBase<TGridItem>? _displayOptionsForColumn;
    ...
    /// <summary>
    /// Constructs an instance of <see cref="BitQuickGrid{TGridItem}"/>.
    /// </summary>
    public BitQuickGrid()
    {
        _columns = new();
        _internalGridContext = new(this);
        _currentPageItemsChanged = new(EventCallback.Factory.Create<BitQuickGridPaginationState>(this, RefreshDataCoreAsync));
    ...
    /// components derived from the <see cref="BitQuickGridColumnBase{TGridItem}"/> base class.
    /// </summary>
    ...
    /// You should supply either <see cref="Items"/> or <see cref="ItemsProvider"/>, but not both.
    /// </summary>
    [Parameter] public BitQuickGridItemsProvider<TGridItem>? ItemsProvider { get; set; }
    ...
    /// <summary>
    /// Optionally links this <see cref="BitQuickGrid{TGridItem}"/> instance with a <see cref="BitQuickGridPaginationState"/> model,
    /// causing the grid to fetch and render only the current page of data.
    ///
    /// This is normally used in conjunction with a <see cref="BitQuickGridPaginator"/> component or some other UI logic
    /// that displays and updates the supplied <see cref="BitQuickGridPaginationState"/> instance.
    /// </summary>
    ...
    /// <summary>
    /// Optional template to customize row rendering. Receives <see cref="BitQuickGridRowTemplateArgs{TGridItem}"/> with
    /// <see cref="BitQuickGridRowTemplateArgs{T}.OriginalRow"/> set to the default row content; call it to render the original cells or replace with custom content.
    /// </summary>
    [Parameter] public RenderFragment<BitQuickGridRowTemplateArgs<TGridItem>>? RowTemplate { get; set; }
    ...
    /// <param name="direction">The direction of sorting. If the value is <see cref="BitQuickGridSortDirection.Auto"/>, then it will toggle the direction on each call.</param>
    /// <returns>A <see cref="Task"/> representing the completion of the operation.</returns>
    public Task SortByColumnAsync(BitQuickGridColumnBase<TGridItem> column, BitQuickGridSortDirection direction = BitQuickGridSortDirection.Auto)
    {
    ...
    /// <summary>
    /// Displays the <see cref="BitQuickGridColumnBase{TGridItem}.ColumnOptions"/> UI for the specified column, closing any other column
    ...
    public void ShowColumnOptions(BitQuickGridColumnBase<TGridItem> column)
    {
    ...
    internal void AddColumn(BitQuickGridColumnBase<TGridItem> column, BitQuickGridSortDirection? isDefaultSortDirection)
    {
    ...
    private async ValueTask<BitQuickGridItemsProviderResult<TGridItem>> ResolveItemsRequestAsync(BitQuickGridItemsProviderRequest<TGridItem> request)
    {
        ...
        {
            return BitQuickGridItemsProviderResult.From(Array.Empty<TGridItem>(), 0);
        }
    }

    private string AriaSortValue(BitQuickGridColumnBase<TGridItem> column)
        => _sortByColumn == column
            ? (_sortByAscending ? "ascending" : "descending")
            : "none";

    private string? ColumnHeaderClass(BitQuickGridColumnBase<TGridItem> column)
        => _sortByColumn == column
    ...
    private static string? ColumnClass(BitQuickGridColumnBase<TGridItem> column) => column.Align switch
    {
