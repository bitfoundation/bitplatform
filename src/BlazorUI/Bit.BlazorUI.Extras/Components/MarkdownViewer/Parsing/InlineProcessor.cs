...
/// to the pipeline's <see cref="InlineParser"/>s at their trigger characters and
/// collecting delimiter runs, then resolves the delimiters into emphasis-like nodes
/// via the pipeline's <see cref="DelimiterProcessor"/>s.
/// </summary>
public sealed class InlineProcessor
{
    private readonly StringBuilder _literal = new();
    private readonly List<Tok> _tokens = new();

    internal InlineProcessor(BitMarkdownPipeline pipeline) => Pipeline = pipeline;
    ...
    public List<MarkdownNode> Parse(string text)
    {
        Text = text;
        Pos = 0;
        _literal.Clear();
        _tokens.Clear();
        Scan();
        DelimiterResolver.Process(_tokens, Pipeline);
        return ToNodes(_tokens);
    }

    /// <summary>Parses inline content in an isolated child processor (e.g. for a link label).</summary>
    public List<MarkdownNode> ParseInlines(string text) => new InlineProcessor(Pipeline).Parse(text);
    ...
    public void AppendNode(MarkdownNode node)
    {
        Flush();
    ...
    internal static List<MarkdownNode> ToNodes(List<Tok> tokens)
    {
        ...
        foreach (var t in tokens)
        {
            ...
            {
                case TokKind.Text:
                    if (t.Text.Length > 0) result.Add(new TextNode(t.Text));
                    break;
                case TokKind.Node:
                    if (t.Node is not null) result.Add(t.Node);
                    break;
                case TokKind.Delim:
                    if (t.Count > 0) result.Add(new TextNode(new string(t.DelimChar, t.Count)));
                    break;
            }
        }
        return result;
