...
public class BitNumericTextFieldUShortTests : BunitTestContext
{
    ...
    {
        var component = RenderComponent<BitNumericTextField<ushort>>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    {
        ushort onChangeEventValue = 0;
        var component = RenderComponent<BitNumericTextField<ushort>>(parameters =>
        {
            parameters.Add(p => p.OnChange, (ushort value) => onChangeEventValue = value);
        });
        ...
        Assert.AreEqual(onChangeEventValue, (ushort)inputValue);
    }
    ...
    public void BitNumericTextFieldInputShouldHaveCorrectAriaValueNow(int? ariaValueNow, string suffix)
    {
        ...
        var input = component.Find("input");
        var expectedResult = ariaValueNow.HasValue ? (ushort?)ariaValueNow : suffix.HasNoValue() ? component.Instance.Value : null;
    ...
    public void BitNumericTextFieldContinuousIncrementOnPointerDownTest(int defaultValue, int step, int max, int timeout)
    {
        ...
        var expectedResult = (ushort)defaultValue + (ushort)step * (ushort)(initialIncrementCount + continuousIncrementCount);
        incrementButton.PointerDown();
    ...
    public void BitNumericTextFieldContinuousDecrementOnPointerDownTest(int defaultValue, int step, int min, int timeout)
    {
        ...
        var expectedResult = (ushort)defaultValue - (ushort)step * (ushort)(initialDecrementCount + continuousDecrementCount);
        incrementButton.PointerDown();
    ...
    private ushort? Normalize(ushort? value, int precision) =>
        value.HasValue ? (ushort?)Math.Round((double)value.Value, precision) : null;
