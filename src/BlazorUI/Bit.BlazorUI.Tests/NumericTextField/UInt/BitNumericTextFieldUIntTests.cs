...
[TestClass]
public class BitNumericTextFieldUIntTests : BunitTestContext
{
    private uint BitNumericTextFieldTwoWayBoundValue;
    ...
    {
        var component = RenderComponent<BitNumericTextFieldUIntTest>(parameters =>
        {
    ...
    {
        var component =
            RenderComponent<BitNumericTextFieldUIntTest>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    public void BitNumericTextFieldShouldHaveCorrectMaxMin(int? min, int? max)
    {
        var component = RenderComponent<BitNumericTextFieldUIntTest>(parameters =>
        {
            parameters.Add(p => p.Min, (uint?)min);
            parameters.Add(p => p.Max, (uint?)max);
        });

        var input = component.Find("input");
        uint? expectedMinValue = null;
        uint? expectedMaxValue = null;

        if (max is not null)
        {
            expectedMaxValue = (uint)max.Value;
        }

        if (min is not null)
        {
            expectedMinValue = (uint)min.Value;
        }
    ...
    {
        var component = RenderComponent<BitNumericTextFieldUIntTest>();
    ...
    public void BitNumericTextFieldOnChangeTest(int inputValue)
    {
        ...
        changeArgs.Value = (uint)inputValue;
        input.Change((uint)inputValue);
        input.Blur();

        Assert.AreEqual((uint)inputValue, component.Instance.OnChangeEventValue);
    }
    ...
    public void BitNumericTextFieldInputShouldHaveCorrectAriaValueNow(int? ariaValueNow, string suffix)
    {
        var component = RenderComponent<BitNumericTextFieldUIntTest>(parameters =>
        {
            parameters.Add(p => p.AriaValueNow, (uint?)ariaValueNow);
            parameters.Add(p => p.Suffix, suffix);
        });

        var input = component.Find("input");
        var expectedResult = ariaValueNow is not null ? (uint?)ariaValueNow :
    ...
    public void BitNumericTextFieldIncrementButtonClickTest(int defaultValue, int step, int max)
    {
        var component = RenderComponent<BitNumericTextField<uint>>(parameters =>
        {
            parameters.Add(p => p.Step, (uint)step);
            parameters.Add(p => p.Max, (uint)max);
            parameters.Add(p => p.DefaultValue, (uint)defaultValue);
            parameters.Add(p => p.Arrows, true);
        });
        ...
        var inputValue = input.GetAttribute("value");
        var expectedResult = (uint)defaultValue + (uint)step <= (uint)max
            ? (uint)defaultValue + (uint)step
            : (uint)defaultValue;
    ...
    public void BitNumericTextFieldEnterKeyDownTest(int defaultValue, int min, int max, string userInput)
    {
        ...
        var isNumber = uint.TryParse(userInput, out var numericValue);
        if (isNumber)
        {
            expectedResult = Normalize(numericValue, 1);
            if (expectedResult > max) expectedResult = (uint)max;
            if (expectedResult < min) expectedResult = (uint)min;
        }
        else
        {
            expectedResult = (uint)defaultValue;
        }
    ...
    public void BitNumericTextFieldPrecisionTest(int min, int max, int step, string userInput)
    {
        ...
        input.Blur();
        var inputValue = component.Instance.Value;
        var precision = CalculatePrecision((uint)step);
        var expectedResult = Normalize(uint.Parse(userInput), precision);
        if (expectedResult > max) expectedResult = (uint)max;
        if (expectedResult < min) expectedResult = (uint)min;
    ...
    public void BitNumericTextFieldTwoWayBoundWithCustomHandlerShouldWorkCurrect(int value, int countOfIncrements,
        int step)
    {
        BitNumericTextFieldTwoWayBoundValue = (uint)value;
        ...
        Assert.AreEqual((uint)expectedValue, BitNumericTextFieldTwoWayBoundValue);
    }
    ...
    public void BitNumericTextFieldContinuousIncrementOnMouseDownTest(int defaultValue, int step, int max, int timeout)
    {
        ...
        var expectedResult = (uint)defaultValue + (uint)step * (uint)(initialIncrementCount + continuousIncrementCount);
        incrementButton.MouseDown();
    ...
    public void BitNumericTextFieldContinuousDecrementOnMouseDownTest(int defaultValue, int step, int min, int timeout)
    {
        ...
        var expectedResult = (uint)defaultValue - (uint)step * (uint)(initialDecrementCount + continuousDecrementCount);
        incrementButton.MouseDown();
    ...
    {
        var component = RenderComponent<BitNumericTextFieldUIntValidationTest>(parameters =>
        {
            parameters.Add(p => p.TestModel, new BitNumericTextFieldUIntTestModel { Value = (uint)value });
    ...
    private uint? Normalize(uint? value, int precision) =>
        value is null ? null : (uint?)Math.Round((double)value.Value, precision);

    private int CalculatePrecision(uint value)
    {
    ...
    private void HandleValueChanged(uint value)
    {
