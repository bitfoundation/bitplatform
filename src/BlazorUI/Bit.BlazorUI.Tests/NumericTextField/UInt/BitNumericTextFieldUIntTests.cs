...
public class BitNumericTextFieldUIntTests : BunitTestContext
{
    ...
    {
        var component = RenderComponent<BitNumericTextField<uint>>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    {
        uint onChangeEventValue = 0;
        var component = RenderComponent<BitNumericTextField<uint>>(parameters =>
        {
            parameters.Add(p => p.OnChange, (uint value) => onChangeEventValue = value);
        });
        ...
        Assert.AreEqual(onChangeEventValue, (uint)inputValue);
    }
    ...
    public void BitNumericTextFieldInputShouldHaveCorrectAriaValueNow(int? ariaValueNow, string suffix)
    {
        ...
        var input = component.Find("input");
        var expectedResult = ariaValueNow.HasValue ? (uint?)ariaValueNow : suffix.HasNoValue() ? component.Instance.Value : null;
    ...
    private uint? Normalize(uint? value, int precision) =>
        value.HasValue ? (uint?)Math.Round((double)value.Value, precision) : null;
