...
public class BitNumericTextFieldByteTests : BunitTestContext
{
    ...
    {
        var component = RenderComponent<BitNumericTextField<byte>>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    {
        byte onChangeEventValue = 0;
        var component = RenderComponent<BitNumericTextField<byte>>(parameters =>
        {
            parameters.Add(p => p.OnChange, (byte value) => onChangeEventValue = value);
        });
        ...
        Assert.AreEqual(onChangeEventValue, (byte)inputValue);
    }
    ...
    public void BitNumericTextFieldInputShouldHaveCorrectAriaValueNow(int? ariaValueNow, string suffix)
    {
        ...
        var input = component.Find("input");
        var expectedResult = ariaValueNow.HasValue ? (byte?)ariaValueNow : suffix.HasNoValue() ? component.Instance.Value : null;
    ...
    private byte? Normalize(byte? value, int precision) =>
        value.HasValue ? (byte?)Math.Round((double)value.Value, precision) : null;
