...
[TestClass]
public class BitNumericTextFieldByteTests : BunitTestContext
{
    private byte BitNumericTextFieldTwoWayBoundValue;
    ...
    {
        var component = RenderComponent<BitNumericTextFieldByteTest>(parameters =>
        {
    ...
    {
        var component =
            RenderComponent<BitNumericTextFieldByteTest>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    public void BitNumericTextFieldShouldHaveCorrectMaxMin(int? min, int? max)
    {
        var component = RenderComponent<BitNumericTextFieldByteTest>(parameters =>
        {
            parameters.Add(p => p.Min, (byte?)min);
            parameters.Add(p => p.Max, (byte?)max);
        });

        var input = component.Find("input");
        byte? expectedMinValue = null;
        byte? expectedMaxValue = null;

        if (max is not null)
        {
            expectedMaxValue = (byte?)max.Value;
        }

        if (min is not null)
        {
            expectedMinValue = (byte?)min.Value;
        }
    ...
    {
        var component = RenderComponent<BitNumericTextFieldByteTest>();
    ...
    public void BitNumericTextFieldOnChangeTest(int inputValue)
    {
        ...
        changeArgs.Value = (byte)inputValue;
        input.Change((byte)inputValue);
        input.Blur();

        Assert.AreEqual((byte)inputValue, component.Instance.OnChangeEventValue);
    }
    ...
    public void BitNumericTextFieldInputShouldHaveCorrectAriaValueNow(int? ariaValueNow, string suffix)
    {
        var component = RenderComponent<BitNumericTextFieldByteTest>(parameters =>
        {
            parameters.Add(p => p.AriaValueNow, (byte?)ariaValueNow);
            parameters.Add(p => p.Suffix, suffix);
        });

        var input = component.Find("input");
        var expectedResult = ariaValueNow is not null ? (byte?)ariaValueNow :
    ...
    public void BitNumericTextFieldIncrementButtonClickTest(int defaultValue, int step, int max)
    {
        var component = RenderComponent<BitNumericTextField<byte>>(parameters =>
        {
            parameters.Add(p => p.Step, (byte)step);
            parameters.Add(p => p.Max, (byte)max);
            parameters.Add(p => p.DefaultValue, (byte)defaultValue);
            parameters.Add(p => p.Arrows, true);
        });
    ...
    public void BitNumericTextFieldEnterKeyDownTest(int defaultValue, int min, int max, string userInput)
    {
        ...
        var isNumber = byte.TryParse(userInput, out var numericValue);
        if (isNumber)
        {
            expectedResult = Normalize(numericValue, 1);
            if (expectedResult > max) expectedResult = (byte)max;
            if (expectedResult < min) expectedResult = (byte)min;
        }
        else
        {
            expectedResult = (byte)defaultValue;
        }
    ...
    public void BitNumericTextFieldPrecisionTest(int min, int max, int step, string userInput)
    {
        ...
        input.Blur();
        var inputValue = component.Instance.Value;
        var precision = CalculatePrecision((byte)step);
        var expectedResult = Normalize(byte.Parse(userInput), precision);
        if (expectedResult > max) expectedResult = (byte)max;
        if (expectedResult < min) expectedResult = (byte)min;
    ...
    public void BitNumericTextFieldTwoWayBoundWithCustomHandlerShouldWorkCorrect(int value, int countOfIncrements, int step)
    {
        ...
        Assert.AreEqual((byte)expectedValue, BitNumericTextFieldTwoWayBoundValue);
    }
    ...
    {
        var component = RenderComponent<BitNumericTextFieldByteValidationTest>(parameters =>
        {
            parameters.Add(p => p.TestModel, new BitNumericTextFieldByteTestModel { Value = (byte)value });
    ...
    private byte? Normalize(byte? value, int precision) =>
        value is null ? null : (byte?)Math.Round((double)value.Value, precision);

    private int CalculatePrecision(byte value)
    {
    ...
    private void HandleValueChanged(byte value)
    {
