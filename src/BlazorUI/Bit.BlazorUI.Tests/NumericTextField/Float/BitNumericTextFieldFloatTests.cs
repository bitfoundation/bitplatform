...
public class BitNumericTextFieldFloatTests : BunitTestContext
{
    ...
    {
        var component = RenderComponent<BitNumericTextField<float>>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    public void BitNumericTextFieldShouldHaveCorrectMaxMin(double? min, double? max)
    {
        var component = RenderComponent<BitNumericTextField<float?>>(parameters =>
        {
            parameters.Add(p => p.Min, (float?)min);
            parameters.Add(p => p.Max, (float?)max);
        });
        ...
        if (max.HasValue)
        {
            expectedMaxValue = (float?)max.Value;
        }

        if (min.HasValue)
        {
            expectedMinValue = (float?)min.Value;
        }
    ...
    {
        float onChangeEventValue = 0;
        var component = RenderComponent<BitNumericTextField<float>>(parameters =>
        {
            parameters.Add(p => p.Precision, precision);
            parameters.Add(p => p.OnChange, (float value) => onChangeEventValue = value);
        });
        ...
        Assert.AreEqual(onChangeEventValue, (float)inputValue);
    }
    ...
    public void BitNumericTextFieldInputShouldHaveCorrectAriaValueNow(double? ariaValueNow, string suffix)
    {
        var component = RenderComponent<BitNumericTextField<float?>>(parameters =>
        {
            parameters.Add(p => p.AriaValueNow, (float?)ariaValueNow);
            parameters.Add(p => p.Suffix, suffix);
        });

        var input = component.Find("input");
        var expectedResult = ariaValueNow.HasValue ? (float?)ariaValueNow : suffix.HasNoValue() ? component.Instance.Value : null;
    ...
    public void BitNumericTextFieldIncrementButtonClickTest(double defaultValue, double step, double max)
    {
        var component = RenderComponent<BitNumericTextField<float>>(parameters =>
        {
            parameters.Add(p => p.Step, (float)step);
            parameters.Add(p => p.Max, (float)max);
            parameters.Add(p => p.DefaultValue, (float)defaultValue);
            parameters.Add(p => p.ShowArrows, true);
        });
    ...
    public void BitNumericTextFieldEnterKeyDownTest(double defaultValue, double min, double max, string userInput, int precision)
    {
        ...
        if (isNumber)
        {
            ...
            if (expectedResult > (float)max) expectedResult = (float)max;
            if (expectedResult < (float)min) expectedResult = (float)min;
        }
        else
        {
            expectedResult = (float)defaultValue;
        }
    ...
    public void BitNumericTextFieldPrecisionTest(double min, double max, double step, string userInput)
    {
        ...
        input.Blur();
        var inputValue = component.Instance.Value;
        var precision = CalculatePrecision((float)step);
    ...
    {
        BitNumericTextFieldTwoWayBoundValue = (float)value;
        ...
        var normalizeValue = Normalize((float)expectedValue, precision);
    ...
    public void BitNumericTextFieldContinuousIncrementOnPointerDownTest(double defaultValue, double step, double max, int timeout)
    {
        ...
        var expectedResult = (float)defaultValue + (float)step * (float)(initialIncrementCount + continuousIncrementCount);
        incrementButton.PointerDown();
    ...
    public void BitNumericTextFieldValidationFormTest(double value)
    {
        ...
        {
            parameters.Add(p => p.TestModel, new BitNumericTextFieldFloatTestModel { Value = (float?)value });
    ...
    private float? Normalize(float? value, int precision) =>
        value.HasValue ? (float?)Math.Round((double)value.Value, precision) : null;
