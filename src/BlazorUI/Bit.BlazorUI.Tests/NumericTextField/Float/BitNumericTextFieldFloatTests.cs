...
public class BitNumericTextFieldFloatTests : BunitTestContext
{
    ...
    {
        var component =
            RenderComponent<BitNumericTextFieldFloatTest>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    [DataTestMethod,
     ...
     DataRow(5.3f, null),
     DataRow(null, 100.5f),
     DataRow(0.0f, 100.78f),
     DataRow(50.56f, 1.12f)
    ]
    ...
    [DataTestMethod,
     DataRow(3.2216f, 1.1322f, 12.36f),
     DataRow(8.2216f, 2.1322f, 10.36f),
     DataRow(8.2216f, 1.1322f, 8.369f),
     DataRow(8.2216f, 2.1322f, 9.369f),
     DataRow(8.2216f, 5.1322f, 9.369f)
    ]
    ...
    [DataTestMethod,
      DataRow(0, 100f, 1, "25"),
      DataRow(0.65f, 100, 0.1f, "25.68"),
      DataRow(0, 100.89f, 0.25f, "12.6"),
      DataRow(0, 10, 0.25f, "12.6"),
      DataRow(13, 100, 0.25f, "12.6")
     ]
    ...
    public void BitNumericTextFieldTwoWayBoundWithCustomHandlerShouldWorkCorrect(float value, int countOfIncrements, float step, int precision)
    {
    ...
    public void BitNumericTextFieldContinuousIncrementOnPointerDownTest(float defaultValue, float step, float max, int timeout)
    {
        var component = RenderComponent<BitNumericTextField<float>>(parameters =>
        {
            parameters.Add(p => p.Step, step);
    ...
    public void BitNumericTextFieldContinuousDecrementOnPointerDownTest(float defaultValue, float step, float min, int timeout)
    {
        var component = RenderComponent<BitNumericTextField<float>>(parameters =>
        {
            parameters.Add(p => p.Step, step);
    ...
    public void BitNumericTextFieldValidationInvalidCssClassTest(float value)
    {
        var component = RenderComponent<BitNumericTextFieldFloatValidationTest>(parameters =>
        {
            parameters.Add(p => p.TestModel, new BitNumericTextFieldFloatTestModel { Value = value });
    ...
    private float? Normalize(float? value, int precision) =>
        value is null ? null : (float?)Math.Round(value.Value, precision);
