...
[TestClass]
public class BitNumericTextFieldSByteTests : BunitTestContext
{
    private sbyte BitNumericTextFieldTwoWayBoundValue;
    ...
    {
        var component = RenderComponent<BitNumericTextField<sbyte>>(parameters =>
        {
    ...
    {
        var component = RenderComponent<BitNumericTextField<sbyte>>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    public void BitNumericTextFieldShouldHaveCorrectMaxMin(int? min, int? max)
    {
        var component = RenderComponent<BitNumericTextField<sbyte?>>(parameters =>
        {
            parameters.Add(p => p.Min, (sbyte?)min);
            parameters.Add(p => p.Max, (sbyte?)max);
        });

        var input = component.Find("input");
        sbyte? expectedMinValue = null;
        sbyte? expectedMaxValue = null;

        if (max.HasValue)
        {
            expectedMaxValue = (sbyte?)max.Value;
        }

        if (min.HasValue)
        {
            expectedMinValue = (sbyte?)min.Value;
        }
    ...
    {
        sbyte onChangeEventValue = 0;
        var component = RenderComponent<BitNumericTextField<sbyte>>(parameters =>
        {
            parameters.Add(p => p.OnChange, (sbyte value) => onChangeEventValue = value);
        });

        var input = component.Find("input");
        input.Change((sbyte)inputValue);
        input.Blur();

        Assert.AreEqual(onChangeEventValue, (sbyte)inputValue);
    }
    ...
    public void BitNumericTextFieldInputShouldHaveCorrectAriaValueNow(int? ariaValueNow, string suffix)
    {
        var component = RenderComponent<BitNumericTextField<sbyte?>>(parameters =>
        {
            parameters.Add(p => p.AriaValueNow, (sbyte?)ariaValueNow);
            parameters.Add(p => p.Suffix, suffix);
        });

        var input = component.Find("input");
        var expectedResult = ariaValueNow.HasValue ? (sbyte?)ariaValueNow : suffix.HasNoValue() ? component.Instance.Value : null;
    ...
    public void BitNumericTextFieldIncrementButtonClickTest(int defaultValue, int step, int max)
    {
        var component = RenderComponent<BitNumericTextField<sbyte>>(parameters =>
        {
            parameters.Add(p => p.Step, (sbyte)step);
            parameters.Add(p => p.Max, (sbyte)max);
            parameters.Add(p => p.DefaultValue, (sbyte)defaultValue);
            parameters.Add(p => p.ShowArrows, true);
        });
        ...
        var inputValue = input.GetAttribute("value");
        var expectedResult = (sbyte)defaultValue + (sbyte)step <= (sbyte)max
            ? (sbyte)defaultValue + (sbyte)step
            : (sbyte)defaultValue;
    ...
    public void BitNumericTextFieldDecrementButtonClickTest(int defaultValue, int step, int min)
    {
        ...
        var inputValue = input.GetAttribute("value");
        var expectedResult = (sbyte)defaultValue - (sbyte)step >= (sbyte)min
            ? (sbyte)defaultValue - (sbyte)step
            : (sbyte)defaultValue;
    ...
    public void BitNumericTextFieldEnterKeyDownTest(int defaultValue, int min, int max, string userInput)
    {
        ...
        sbyte? expectedResult = 0;
        var isNumber = sbyte.TryParse(userInput, out var numericValue);
        if (isNumber)
        {
            expectedResult = Normalize(numericValue, 1);
            if (expectedResult > max) expectedResult = (sbyte)max;
            if (expectedResult < min) expectedResult = (sbyte)min;
        }
        else
        {
            expectedResult = (sbyte)defaultValue;
        }
    ...
    public void BitNumericTextFieldPrecisionTest(int min, int max, int step, string userInput)
    {
        ...
        input.Blur();
        var inputValue = component.Instance.Value;
        var precision = CalculatePrecision((sbyte)step);
        var expectedResult = Normalize(sbyte.Parse(userInput), precision);
        if (expectedResult > max) expectedResult = (sbyte)max;
        if (expectedResult < min) expectedResult = (sbyte)min;
    ...
    {
        BitNumericTextFieldTwoWayBoundValue = (sbyte)value;
        ...
        Assert.AreEqual((sbyte)expectedValue, BitNumericTextFieldTwoWayBoundValue);
    }
    ...
    public void BitNumericTextFieldContinuousIncrementOnPointerDownTest(int defaultValue, int step, int max, int timeout)
    {
        ...
        var expectedResult = (sbyte)defaultValue + (sbyte)step * (sbyte)(initialIncrementCount + continuousIncrementCount);
        incrementButton.PointerDown();
    ...
    public void BitNumericTextFieldContinuousDecrementOnPointerDownTest(int defaultValue, int step, int min, int timeout)
    {
        ...
        var expectedResult = (sbyte)defaultValue - (sbyte)step * (sbyte)(initialDecrementCount + continuousDecrementCount);
        incrementButton.PointerDown();
    ...
    {
        var component = RenderComponent<BitNumericTextFieldSByteValidationTest>(parameters =>
        {
            parameters.Add(p => p.TestModel, new BitNumericTextFieldSByteTestModel { Value = (sbyte)value });
        ...
        if (isValid)
        {
            input.Change((sbyte)2);
        }
        else
        {
            input.Change((sbyte)8);
        }
    ...
    public void BitNumericTextFieldValidationInvalidHtmlAttributeTest(int value)
    {
        ...
        if (isInvalid)
        {
            input.Change((sbyte)10);
            input.Blur();
            Assert.IsFalse(input.HasAttribute("aria-invalid"));
        }
        else
        {
            input.Change((sbyte)4);
            input.Blur();
    ...
    private sbyte? Normalize(sbyte? value, int precision) =>
        value.HasValue ? (sbyte?)Math.Round((double)value.Value, precision) : null;

    private int CalculatePrecision(sbyte value)
    {
    ...
    private void HandleValueChanged(sbyte value)
