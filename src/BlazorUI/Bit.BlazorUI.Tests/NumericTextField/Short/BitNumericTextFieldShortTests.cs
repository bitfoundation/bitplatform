...
public class BitNumericTextFieldShortTests : BunitTestContext
{
    ...
    {
        var component = RenderComponent<BitNumericTextField<short>>(parameters => parameters.Add(p => p.Suffix, suffix));
    ...
    {
        short onChangeEventValue = 0;
        var component = RenderComponent<BitNumericTextField<short>>(parameters =>
        {
            parameters.Add(p => p.OnChange, (short value) => onChangeEventValue = value);
        });
        ...
        Assert.AreEqual(onChangeEventValue, (short)inputValue);
    }
    ...
    public void BitNumericTextFieldInputShouldHaveCorrectAriaValueNow(int? ariaValueNow, string suffix)
    {
        ...
        var input = component.Find("input");
        var expectedResult = ariaValueNow.HasValue ? (short?)ariaValueNow : suffix.HasNoValue() ? component.Instance.Value : null;
    ...
    public void BitNumericTextFieldContinuousIncrementOnPointerDownTest(int defaultValue, int step, int max, int timeout)
    {
        ...
        var expectedResult = (short)defaultValue + (short)step * (short)(initialIncrementCount + continuousIncrementCount);
        incrementButton.PointerDown();
    ...
    public void BitNumericTextFieldContinuousDecrementOnPointerDownTest(int defaultValue, int step, int min, int timeout)
    {
        ...
        var expectedResult = (short)defaultValue - (short)step * (short)(initialDecrementCount + continuousDecrementCount);
        incrementButton.PointerDown();
    ...
    private short? Normalize(short? value, int precision) =>
        value.HasValue ? (short?)Math.Round((double)value.Value, precision) : null;
