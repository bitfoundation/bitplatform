...
public class BitPaginationTests : BunitTestContext
{
    ...
    [TestMethod]
    public void BitPaginationShouldNotMoveTheSelectionForAModifiedClickOnAClickableEllipsisLink()
    {
        var page = 1;

        var comp = RenderComponent<BitPagination>(parameters =>
        {
            parameters.Add(p => p.Count, 100);
            parameters.Add(p => p.ClickableEllipsis, true);
            parameters.Add(p => p.GetPageHref, (int n) => $"/results?page={n}");
            parameters.Bind(p => p.SelectedPage, page, v => page = v);
        });

        // The gap is a link along with the rest, so the modifier leaves this pagination alone there as well.
        comp.FindAll(".bit-pgn-elb").Last().Click(new Microsoft.AspNetCore.Components.Web.MouseEventArgs { CtrlKey = true });
        Assert.AreEqual(1, page);

        comp.FindAll(".bit-pgn-elb").Last().Click();
        Assert.AreNotEqual(1, page);
    }
