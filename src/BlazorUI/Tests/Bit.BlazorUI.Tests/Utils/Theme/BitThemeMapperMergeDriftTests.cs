...
public sealed class BitThemeMapperMergeDriftTests
{
    ...
    /// The traversal is shared with <see cref="BitThemeModelCoverageTests"/> via
    /// <see cref="BitThemeTestGraph"/> so both contracts agree on what the model graph is.
    /// </summary>
    private static BitTheme BuildSentinelTheme(string prefix = "sentinel")
    {
        var theme = new BitTheme();
        BitThemeTestGraph.FillStringLeavesWithSentinels(theme, prefix);
        return theme;
    }
}
