...
- **Size**: control heights per size class are `$siz-ctrl-sm/md/lg` (also for 32px icon-button
  ...
  progress tracks `$siz-track-sm/md/lg`, scrolling popup lists `$siz-popup-max-height`.
- **Spacing & layout**: dialogs and message boxes inset their content with `$spa-dialog`, and their
  ...
  (never a literal `row` / `flex-end` in a dialog footer — Cupertino stacks them).
...
The packaged Material and Cupertino presets live in `Styles/Material` and `Styles/Cupertino` as
override-only bundles (`bit.blazorui.material.css` / `bit.blazorui.cupertino.css`, linked after the
core stylesheet). Their `colors.*.scss` palettes are GENERATED by the library's seed-derivation
pipeline — regenerate them, never hand-edit — while `tokens.*.scss` / `components.*.scss` hold the
hand-written shape/size/typography/motion and per-component overrides.
