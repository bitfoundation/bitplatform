...
/// <summary>
/// The CSS &lt;named-color&gt; keywords, so that a color handed to the picker as "tomato" is understood
/// the same way a browser understands it in a stylesheet.
/// </summary>
/// <remarks>
/// The values are packed as 0xRRGGBB. "transparent" is not here: it carries an alpha of zero, which
/// does not fit the packing, and is handled separately by the parser.
/// </remarks>
internal static class BitCssNamedColors
{
    /// <summary>
    /// Looks a CSS color keyword up, case-insensitively as CSS itself does.
    /// </summary>
    public static bool TryGet(string name, out uint rgb) => Map.TryGetValue(name, out rgb);
