...
declare type MarkedToken = (Tokens.Blockquote | Tokens.Br | Tokens.Code | Tokens.Codespan | Tokens.Def | Tokens.Del | Tokens.Em | Tokens.Escape | Tokens.Heading | Tokens.Hr | Tokens.HTML | Tokens.Image | Tokens.Link | Tokens.List | Tokens.ListItem | Tokens.Paragraph | Tokens.Space | Tokens.Strong | Tokens.Table | Tokens.Tag | Tokens.Text);
declare type Token = (MarkedToken | Tokens.Generic);
...
declare type Links = Record<string, Pick<Tokens.Link | Tokens.Image, "href" | "title">>;
...
declare interface TokenizerThis {
    lexer: _Lexer;
}
declare type TokenizerExtensionFunction = (this: TokenizerThis, src: string, tokens: Token[] | TokensList) => Tokens.Generic | undefined;
declare type TokenizerStartFunction = (this: TokenizerThis, src: string) => number | void;
declare interface TokenizerExtension {
    name: string;
    level: "block" | "inline";
...
declare interface RendererThis {
    parser: _Parser;
}
declare type RendererExtensionFunction = (this: RendererThis, token: Tokens.Generic) => string | false | undefined;
declare interface RendererExtension {
    name: string;
    renderer: RendererExtensionFunction;
}
declare type TokenizerAndRendererExtension = TokenizerExtension | RendererExtension | (TokenizerExtension & RendererExtension);
declare type HooksApi = Omit<_Hooks, "constructor" | "options" | "block">;
...
declare type RendererApi = Omit<_Renderer, "constructor" | "options" | "parser">;
declare type RendererObject = {
    [K in keyof RendererApi]?: (this: _Renderer, ...args: Parameters<RendererApi[K]>) => ReturnType<RendererApi[K]> | false;
};
declare type TokenizerApi = Omit<_Tokenizer, "constructor" | "options" | "rules" | "lexer">;
declare type TokenizerObject = {
...
declare interface MarkedOptions extends Omit<MarkedExtension, "hooks" | "renderer" | "tokenizer" | "extensions" | "walkTokens"> {
    /**
     * Hooks are methods that hook into some part of marked.
     */
    hooks?: _Hooks | null;
...
declare type MaybePromise = void | Promise<void>;
...
declare const walkTokens: (tokens: Token[] | TokensList, callback: (token: Token) => MaybePromise | MaybePromise[]) => MaybePromise[];
declare const parseInline: {
    (src: string, options: MarkedOptions & {
        async: true;
    }): Promise<string>;
    (src: string, options: MarkedOptions & {
