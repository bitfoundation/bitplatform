...
internal static class BitMarkdownViewerJsRuntimeExtensions
{
    ...
    {
        return jsRuntime.FastInvoke<bool>("BitBlazorUI.Legacy.MarkdownViewer.checkScriptLoaded", script);
    }
    ...
    public static ValueTask<string> BitMarkdownViewerParse(this IJSRuntime jsRuntime, string markdown, string? middleware)
    {
        return OperatingSystem.IsBrowser() && middleware.HasNoValue()
            ? jsRuntime.FastInvoke<string>("BitBlazorUI.Legacy.MarkdownViewer.parse", markdown)
            : jsRuntime.Invoke<string>("BitBlazorUI.Legacy.MarkdownViewer.parseAsync", markdown, middleware);
    }
}
