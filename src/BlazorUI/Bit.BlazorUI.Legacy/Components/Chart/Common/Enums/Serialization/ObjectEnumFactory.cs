...
internal class ObjectEnumFactory
{
    ...
    /// </summary>
    /// <param name="enumType">The <see cref="BitChartLegacyObjectEnum"/>-type whose factory to get.</param>
    ...
    public static ObjectEnumFactory GetFactory(Type? enumType)
    {
        ...
        if (!typeof(BitChartLegacyObjectEnum).IsAssignableFrom(enumType))
            throw new ArgumentException($"The type '{enumType.FullName}' doesn't inherit from '{typeof(BitChartLegacyObjectEnum).FullName}'");
    ...
    /// <summary>
    /// Creates a new instance of the <see cref="BitChartLegacyObjectEnum"/>-type this factory is for.
    /// If there is no suitable constructor for the <paramref name="value"/>, a
    /// <see cref="NotSupportedException"/> will be thrown.
    /// </summary>
    /// <param name="value">The value used for instantiating the <see cref="BitChartLegacyObjectEnum"/>.</param>
    public BitChartLegacyObjectEnum Create(object value) => Create(value, value.GetType());
    ...
    public BitChartLegacyObjectEnum Create(object value, Type valueType)
    {
        if (_constructorCache.TryGetValue(valueType, out ConstructorInfo? constructor))
        {
            return (BitChartLegacyObjectEnum)constructor.Invoke([value]);
        }

        if (BitChartLegacyObjectEnum.IsSupportedSerializationType(valueType))
        {
        ...
        {
            throw new NotSupportedException($"The type '{valueType}' isn't supported for serialization within {nameof(BitChartLegacyObjectEnum)}.");
        }
    }

    /// <summary>
    /// Checks if a suitable constructor for this <paramref name="contentType"/> exists which
    /// can be used to create a new instance of that <see cref="BitChartLegacyObjectEnum"/>-type.
    /// </summary>
    ...
    private Dictionary<Type, ConstructorInfo> CreateConstructorDictionary()
    {
        ...
        foreach (ConstructorInfo constructor in constructors)
        {
            ...
            if (BitChartLegacyObjectEnum.IsSupportedSerializationType(paramType))
            {
        ...
        if (dict.Count == 0)
        {
            throw new NotSupportedException($"The {nameof(BitChartLegacyObjectEnum)} type '{_enumType.FullName}' doesn't have any " +
