...
<BitQuickGrid Items=""@FilteredItems"" Pagination=""@pagination"" ResizableColumns>
    ...
    <BitQuickGridPropertyColumn Property=""@(c => c.Medals.Total)"" Sortable=""true"" />
</BitQuickGrid>
<BitQuickGridPaginator Value=""@pagination"" />";
    private readonly string example1CsharpCode = @"
private IQueryable<CountryModel> allCountries;
private string typicalSampleNameFilter = string.Empty;
private BitQuickGridPaginationState pagination = new() { ItemsPerPage = 7 };
...
<div class=""custom-grid"">
    ...
    <BitQuickGridPaginator Value=""@pagination"" SummaryFormat=""@(v => $""Total: {v.TotalItemCount}"")"">
        <TextTemplate Context=""state"">@(state.CurrentPageIndex + 1) / @(state.LastPageIndex + 1)</TextTemplate>
    </BitQuickGridPaginator>
</div>";
...
<div class=""grid"">
    <BitQuickGrid @ref=""dataGrid"" ItemsProvider=""@foodRecallProvider"" TGridItem=""FoodRecall"" Virtualize ItemSize=""32"">
        ...
        <BitQuickGridPropertyColumn Property=""@(c => c.RecallingFirm)"" Title=""Company"" />
        <BitQuickGridPropertyColumn Property=""@(c => c.Status)"" />
        <BitQuickGridPropertyColumn Property=""@(c => c.ReportDate)"" Title=""Report Date"" Sortable=""true"" />
    </BitQuickGrid>
</div>
...
BitQuickGrid<FoodRecall>? dataGrid;
string _virtualSampleNameFilter = string.Empty;
BitQuickGridItemsProvider<FoodRecall> foodRecallProvider;
...
protected override async Task OnInitializedAsync()
{
    foodRecallProvider = async req =>
    {
        try
        {
            ...
            return BitQuickGridItemsProviderResult.From(
                                            items: data!.Results,
...
BitQuickGrid<ProductDto>? productsDataGrid;
string _odataSampleNameFilter = string.Empty;
BitQuickGridItemsProvider<ProductDto> productsItemsProvider;
...
<BitQuickGridPaginator Value=""@pagination"" SummaryFormat=""@(v => $""Total: {v.TotalItemCount?.ToString(""N0"")}"")"">
    <TextTemplate Context=""state"">@(state.CurrentPageIndex + 1) / @(state.LastPageIndex + 1)</TextTemplate>
</BitQuickGridPaginator>";
...
BitQuickGrid<ProductDto>? productsDataGrid;
BitQuickGridItemsProvider<ProductDto> productsItemsProvider;
BitQuickGridPaginationState pagination = new() { ItemsPerPage = 7 };
...
<div class=""responsive-grid"">
    <BitQuickGrid Items=""@allCountries"" Pagination=""@pagination"">
        <BitQuickGridPropertyColumn Property=""@(c => c.Name)"" />
        <BitQuickGridPropertyColumn Property=""@(c => c.Medals.Gold)"" />
        <BitQuickGridPropertyColumn Property=""@(c => c.Medals.Silver)"" />
        <BitQuickGridPropertyColumn Property=""@(c => c.Medals.Bronze)"" />
        <BitQuickGridPropertyColumn Property=""@(c => c.Medals.Total)"" />
    </BitQuickGrid>
    <BitQuickGridPaginator Value=""@pagination"" />
</div>";
