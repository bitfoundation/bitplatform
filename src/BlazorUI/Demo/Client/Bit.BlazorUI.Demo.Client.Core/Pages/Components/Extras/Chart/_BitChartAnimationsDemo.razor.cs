...
public partial class _BitChartAnimationsDemo
{
    ...
    private BitChartData LineData() => new()
    {
        Labels = BitChartSampleData.Months.ToList(),
        Datasets =
        {
            new BitChartDataset { Label = "Trend", Data = _data.Datasets[0].Data, BorderColor = "#9966ff",
    ...
    private readonly string barsRazorCode = @"<BitChart Type=""BitChartType.Bar"" Data=""_data"" Options=""Opts()"" />";
...
private static BitChartData Build() => new()
{
    ...
    private readonly string lineDrawRazorCode = @"<BitChart Type=""BitChartType.Line"" Data=""LineData()"" Options=""Opts()"" />";
    private readonly string lineDrawCsharpCode = @"
// Shares the same Opts() (duration/easing/stagger) and the bar dataset's values.
private BitChartData LineData() => new()
{
    Labels = { ""Jan"", ""Feb"", ""Mar"", ""Apr"", ""May"", ""Jun"", ""Jul"" },
    Datasets =
