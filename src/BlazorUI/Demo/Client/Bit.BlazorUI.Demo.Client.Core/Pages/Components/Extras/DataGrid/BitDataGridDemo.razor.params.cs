...
public partial class BitDataGridDemo
{
    ...
    [
        ...
        new() { Name = "CellNavigation", Type = "bool", DefaultValue = "false", Description = "Enables keyboard cell navigation with a roving tabindex (arrows/Home/End/PageUp/PageDown to move, Enter/F2 to edit, Esc to cancel, Space to toggle the row's selection, Ctrl+A to select all, Delete to delete the focused row when Editable)." },
    ...
    [
        ...
        new() { Name = "CopyToClipboardAsync", Type = "Task<int>", DefaultValue = "", Description = "CopyToClipboardAsync(fallbackRow) - copies the selected rows (or the given row when nothing is selected) to the system clipboard as tab-separated text with a header line, and returns how many rows were copied; 0 when the clipboard is unavailable." },
        ...
        new() { Name = "GetState", Type = "BitDataGridState", DefaultValue = "", Description = "Captures the user-adjustable state (page, page size, sorts, filters, groups, column layout) as a serializable snapshot.", LinkType = LinkType.Link, Href = "#BitDataGridState" },
        ...
        new() { Name = "ExportCsvAsync", Type = "Task", DefaultValue = "", Description = "Generates the full (filtered/sorted) data as CSV and triggers a client-side download. Covers all matching rows in every data mode - server/infinite modes fetch them through OnRead/OnLoadMore, tree mode includes collapsed branches." },
        new() { Name = "ToCsv", Type = "string", DefaultValue = "", Description = "Builds a CSV string of the full dataset synchronously - tree mode includes collapsed branches, queryable mode covers all pages. Only server/infinite modes are limited to the loaded rows (their providers are async); use ToCsvAsync there." },
        new() { Name = "ToCsvAsync", Type = "Task<string>", DefaultValue = "", Description = "Builds a CSV string of the full dataset - server/infinite modes issue an OnRead/OnLoadMore request with no paging (Take = null), tree mode includes collapsed branches." },
        new() { Name = "ToExcelAsync", Type = "Task<byte[]>", DefaultValue = "", Description = "Generates the full (filtered/sorted) dataset as an Excel workbook (.xlsx), covering all matching rows in every data mode (like ToCsvAsync). The workbook mirrors the grid's layout: bold frozen header row, column widths, leading frozen columns as a freeze pane and ColSpan cells as merged cells; with ExcelExportStyled it also carries the grid's rendered theme (colors, striping, borders, fonts)." },
        new() { Name = "ExportExcelAsync", Type = "Task", DefaultValue = "", Description = "Generates the .xlsx workbook and triggers a client-side download." },
