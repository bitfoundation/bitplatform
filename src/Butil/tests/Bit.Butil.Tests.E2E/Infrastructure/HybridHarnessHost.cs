...
/// <summary>
/// One Bit.Butil.Tests.Harness.Hybrid window - the pages inside a WinForms BlazorWebView, the same WebView core
/// MAUI uses - driven over the Chrome DevTools Protocol. The host opens WebView2's debugging port when started
/// with <c>--remote-debugging-port</c>, and Playwright attaches to it like to any Chromium: the page it drives is
/// the one inside the app window, running the real hybrid renderer and <c>WebViewJSRuntime</c>.
/// </summary>
public sealed class HybridHarnessHost : IAsyncDisposable
{
    ...
    /// <summary>The app window's page. There is only ever this one.</summary>
    public IPage Page { get; }

    /// <summary>Whether the app is still there to drive: a test can crash the process or close its page.</summary>
    public bool IsUsable => _disposed == 0 && _process.HasExited is false && Page.IsClosed is false;

    public string RecentOutput => _process.RecentOutput;

    public static async Task<HybridHarnessHost> StartAsync(bool lazyScripts)
    {
        if (OperatingSystem.IsWindows() is false)
            throw new PlatformNotSupportedException("The BlazorWebView harness runs on WebView2, which needs Windows.");

        var executable = RepoLayout.HybridHarnessExecutable(E2EEnvironment.Configuration);
        ...
        // A user data folder of its own: WebView2 instances sharing one share a browser process, and the second
        // app's debugging port would never open.
        var userDataFolder = Path.Combine(Path.GetTempPath(), "bit-butil-hybrid-" + Guid.NewGuid().ToString("N"));

        // Passed as arguments the host applies through the WebView2 API: an elevated host (CI runners run as
        // admin) ignores --remote-debugging-port in WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS.
        var process = ChildProcess.Start(executable,
            ["--remote-debugging-port", port.ToString(), "--user-data-folder", userDataFolder, "--scripts", lazyScripts ? "lazy" : "bundle"],
        ...
        catch (Exception exception)
        {
            ...
            throw new InvalidOperationException($"The hybrid harness host did not start: {exception.Message}{Environment.NewLine}{output}", exception);
        }
    }
    ...
    /// <summary>
    /// Deletes a WebView2 user data folder once the app has exited. The browser processes WebView2 spawned can
    /// hold files in it for a moment after the host goes, so a failed delete is retried briefly; it is a temp
    /// folder, so giving up after that leaves nothing that matters. Never throws.
    /// </summary>
