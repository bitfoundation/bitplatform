...
public static class DomTouchEventListenersManager
{
    internal const string InvokeMethodName = "InvokeTouchEvent";

    private static readonly ConcurrentDictionary<Guid, Listener> Listeners = [];

    internal static Guid SetListener(Action<ButilTouchEventArgs> action, string element, object options)
    {
        var id = Guid.NewGuid();
    ...
    internal static Guid[] RemoveListener(Action<ButilTouchEventArgs> action, string element, object options)
    {
    ...
    public static void Invoke(Guid id, ButilTouchEventArgs args)
    {
    ...
    private class Listener
    {
        ...
        public Action<ButilTouchEventArgs> Action { get; set; } = default!;
    }
}
