...
public static class DomPointerEventListenersManager
{
    internal const string InvokeMethodName = "InvokePointerEvent";

    private static readonly ConcurrentDictionary<Guid, Listener> Listeners = [];

    internal static Guid SetListener(Action<ButilPointerEventArgs> action, string element, object options)
    {
        var id = Guid.NewGuid();
    ...
    internal static Guid[] RemoveListener(Action<ButilPointerEventArgs> action, string element, object options)
    {
    ...
    public static void Invoke(Guid id, ButilPointerEventArgs args)
    {
    ...
    private class Listener
    {
        ...
        public Action<ButilPointerEventArgs> Action { get; set; } = default!;
    }
}
