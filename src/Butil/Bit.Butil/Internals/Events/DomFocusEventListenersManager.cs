...
public static class DomFocusEventListenersManager
{
    internal const string InvokeMethodName = "InvokeFocusEvent";

    private static readonly ConcurrentDictionary<Guid, Listener> Listeners = [];

    internal static Guid SetListener(Action<ButilFocusEventArgs> action, string element, object options)
    {
        var id = Guid.NewGuid();
    ...
    internal static Guid[] RemoveListener(Action<ButilFocusEventArgs> action, string element, object options)
    {
    ...
    public static void Invoke(Guid id, ButilFocusEventArgs args)
    {
    ...
    private class Listener
    {
        ...
        public Action<ButilFocusEventArgs> Action { get; set; } = default!;
    }
}
