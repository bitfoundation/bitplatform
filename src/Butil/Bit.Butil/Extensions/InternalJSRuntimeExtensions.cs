...
internal static class InternalJSRuntimeExtensions
{
    ...
    /// touched. A module that was never requested has registered nothing on the page, so there is nothing to tear
    /// down. In bundle mode every module is already on the page, and the call goes through as before.
    /// </remarks>
    internal static ValueTask InvokeTeardown(this IJSRuntime jsRuntime, string identifier)
    {
        if (BitButil.LazyScriptsEnabled && ButilScriptLoader.IsModuleRequested(jsRuntime, identifier) is false) return default;
