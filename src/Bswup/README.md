...
> **Prompting instead of reloading.** By default a finished update activates immediately and
> every open tab reloads onto the new version. Set `AutoReload="false"` on `BswupProgress` to
> get the prompt-then-reload pattern instead: the reload button is shown and the new version
> activates when the user accepts it, so an unprompted reload cannot discard whatever in-page
> state the user has mid-session. First installs are unaffected: they always complete the
> seamless claim-and-start flow (no reload) regardless of this setting.
...
> `service-worker-assets.js` (which Bswup can verify with Subresource Integrity when the
> opt-in `enableIntegrityCheck` setting is enabled), the service-worker script itself cannot
> be integrity-pinned: browsers do not support an
...
- `defaultUrl`: The default page URL, served from cache for navigation requests (the SPA fallback). Defaults to `index.html`; use `/` when using `_Host.cshtml`. The value must match an entry that actually exists in `service-worker-assets.js` or `externalAssets`; the comparison uses *resolved* URLs, so equivalent spellings match (`'index.html'` and `'/index.html'` are the same resource for a root-mounted app - they differ, correctly, for an app mounted on a sub-path). When nothing matches, offline navigation cannot work (navigations silently pass through to the network) and Bswup logs a `defaultUrl ... matches no asset` warning to the console at startup. Navigations whose URL is itself a managed asset are served that asset instead of the default document (**changed in v-10-6-0**): opening `/manifest.json` or an image directly in a tab shows that file, while route URLs (`/counter`, ...) match no asset and still get the app shell. If your host answers the shell URL with a redirect (for example `/` &rarr; `/index.html`, common on Cloudflare Pages, Netlify, and some reverse proxies), Bswup rebuilds that response before serving it to a navigation so the browser does not reject the followed redirect with *"a redirected response was used for a request whose redirect mode is not follow"* - offline deep-link navigation keeps working regardless.
...
- `isPassive`: Enables the Bswup's passive mode. In this mode, the assets won't be cached in advance but rather upon initial request. The asset list is never bulk-downloaded: nothing is precached during install, and the post-start background pass is skipped too, so the cache only ever holds what the running app actually requested. What passive mode buys is a first paint that is never blocked behind a full precache, and no bandwidth spent on assets the app doesn't use; what it costs is offline completeness - a page whose lazy-loaded assets were never requested while online won't work offline. Apps that need the whole manifest cached should leave passive mode off (`mode = 'FullOffline'`).
...
- `AutoReload` (default `true`): reload automatically when an update finishes. Set it to `false` to show the reload button and let the user accept the update instead.
...
- `ShowOnUpdate` (default `true`): drive the splash during a background update - one downloading behind an already-installed, running app - as well as during a first install. Set it to `false` when the splash is a full-viewport take-over that should not paint over a live UI; progress still reaches your `Handler`, and the finished update still surfaces through `AutoReload` or the reload button.
...
- `ChildContent`: replaces the default splash markup with your own. The component keeps initializing automatically (it still emits the `data-bit-bswup-*` config), and the built-in behavior drives your markup through the documented element ids (`bit-bswup-progress-bar`, `bit-bswup-percent`, `bit-bswup-assets`, `bit-bswup-error`, ...) plus the `--bit-bswup-percent` / `--bit-bswup-percent-text` CSS variables set on `#bit-bswup` - use whichever you want driven. The update-ready button (`#bit-bswup-reload`) and its screen-reader status region are always rendered by the component itself, *outside* the overlay, even with custom content - they are what an app running under `AutoReload="false"` relies on to surface a finished update - so a custom splash should not render its own copy of those two; restyle them by id instead.

Behavior worth knowing:
- The splash is painted for a background update as well as a first install, so the user sees an update being fetched. Set `ShowOnUpdate="false"` to keep a take-over splash out of a running app's way. Either way, clicks outside the splash content pass through the overlay (`#bit-bswup` is `pointer-events: none`, its children are not), so an app running underneath - or force-started under a failure panel - stays usable.
...
- The built-in progress UI can now be kept out of the way during background updates: `downloadStarted`/`downloadProgress` payloads carry `firstInstall` so custom handlers can tell the two apart, and the new `ShowOnUpdate="false"` makes the built-in splash first-install only (it still paints for both by default). The reload button lives *outside* the `#bit-bswup` overlay so it can appear without covering the app, it is announced via a `role="status"` live region, and the overlay no longer swallows clicks outside its content.
...
- Passive mode is strictly lazy: the post-start background top-up is skipped entirely, so the asset list is never bulk-downloaded and the cache only ever holds what the running app actually requested. (This matters more than it used to: the first-install claim handshake is now deterministic, so the post-start pass - which previously ran only when an activation race happened to be won - would otherwise pull the whole manifest on every first install.)
...
- **Hand-written splash markup needs one migration step:** if you wrote the splash markup yourself (instead of using `BswupProgress`), move `<button id="bit-bswup-reload">` *outside* the `#bit-bswup` overlay and give it its own `z-index`, and optionally add `<span id="bit-bswup-reload-status" role="status">` (visually hidden) next to it. The built-in handling does not always reveal the overlay when an update finishes - an update already staged at page load never produced a progress event to reveal it, and `ShowOnUpdate="false"` suppresses the reveal outright - so a button left inside it would stay invisible in exactly the cases it is needed. (The `BswupProgress` component ships this layout already - including for custom `ChildContent`.)
