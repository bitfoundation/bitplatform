...
public static partial class BswupScriptCatalog
{
    ...
    private static readonly Dictionary<string, string> _settingRemarks = new(StringComparer.Ordinal)
    {
        ...
        ["isPassive"] = "Passive mode never bulk-downloads the asset list - not during install, and not after Blazor starts either. The cache only ever holds what the running app actually requested. What it buys is a first paint that is never blocked behind a full precache, and no bandwidth spent on unused assets; what it costs is offline completeness - anything never requested while online is not there offline.",
