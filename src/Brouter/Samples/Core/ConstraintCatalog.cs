...
/// <summary>
/// One entry of the constraints sample. <c>Kind</c> is the literal URL segment identifying the
/// route (<c>/c/{Kind}/...</c>), <c>Token</c> the constraint text used in that route's template,
/// and the examples are one value that passes and one that fails.
/// </summary>
...
/// <summary>
/// The single source of truth for the constraint sample: AppRouter declares one
/// <c>/c/{Kind}/{v:Token}</c> route per entry, ConstraintsPage renders the links, and
/// ConstraintResultPage looks entries up by <c>Kind</c>.
/// </summary>
public static class ConstraintCatalog
{
    ...
    [
        ...
        new("slug", "slug", "custom", "Custom: at least 3 chars, letters/digits/dashes only.", "hello-world", "a!"),
