...
public class RouteLifecycleTests : BunitTestContext
{
    ...
    {
        var (cut, brouter) = RenderAt("http://localhost/tl");
    ...
    {
        var (cut, brouter) = RenderAt("http://localhost/ren/1");
        cut.WaitForAssertion(() =>
            StringAssert.Contains(cut.Find("[data-testid=lifecycle]").TextContent, "activations:1"));
    ...
    {
        var (cut, brouter) = RenderAt("http://localhost/ppl/1");
        cut.WaitForAssertion(() =>
            StringAssert.Contains(cut.Find("[data-testid=lifecycle]").TextContent, "activations:1"));
        ...
        cut.WaitForAssertion(() =>
        {
            ...
            StringAssert.Contains(visible.TextContent, "activations:1");
        ...
        cut.WaitForAssertion(() =>
        {
            ...
            var visible = cut.FindAll("[data-testid=lifecycle]").Single(e => e.TextContent.Contains("active:True"));
            StringAssert.Contains(visible.TextContent, "activations:2");
            StringAssert.Contains(visible.TextContent, "first:1");
    ...
    {
        var (cut, brouter) = RenderAt("http://localhost/page");
        cut.WaitForAssertion(() =>
