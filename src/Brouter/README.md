...
builder.Services.AddBitBrouterServices(o =>
{
    ...
    o.RemountOnParameterChange = true;    // default: /item/1 -> /item/2 rebuilds the page (false re-binds the instance)
});
```

Brouter's documentation website lives in [`Bit.Brouter.Demo`](Bit.Brouter.Demo/) - run the
[Server](Bit.Brouter.Demo/Server/) project (`dotnet run`) for the full site: a landing page, a
docs hub under `/docs` with a page per feature area (plus an API reference, recipes and a
troubleshooting guide), and a `/playground` index of live, clickable demo routes. It is a
...
code can be exercised in each render mode.
...
- **Parameter-change semantics, your choice**: by default a navigation that only changes a route's parameter values (`/settings/profile → /settings/account`) rebuilds the route's content, so a page that reads a value once (`OnInitialized`, an uncontrolled `Default*` parameter on a UI component) sees the new one. Set `o.RemountOnParameterChange = false` (or `<Broute RemountOnParameterChange="false">` for a single route) to re-bind the live instance instead and receive the change as `OnRenavigated` - the reuse Blazor's built-in `Router` gives a page, and what Angular and Vue do
...
(`/settings/profile → /settings/account`, `/item/1 → /item/2`). By default Brouter rebuilds the
route's content: the old instance is disposed and a brand-new one mounts, so everything that reads
a value *once* - `OnInitialized`, a child that captures a parameter when it registers with its
parent, an uncontrolled `Default*` parameter on a UI component (`DefaultExpandedKey`,
`DefaultSelectedKey`) - starts from the new values.

The alternative is to keep the component instance: the new values arrive as parameters and the
change is reported as `OnRenavigated` rather than a deactivation plus a fresh activation. That is
the reuse Blazor's built-in `Router` gives a page (its `RouteView` re-parameterizes the existing
component - `OnParametersSet` runs, `OnInitialized` does not), and what Angular and Vue do; it
preserves scroll position, form state and in-flight work across the change, at the cost of every
affected component having to react to the new parameters itself - one that doesn't keeps showing
the value the page was first mounted with. Opt into it with:

```csharp
services.AddBitBrouterServices(o =>
{
    o.RemountOnParameterChange = false; // /item/1 -> /item/2 re-binds the live page instance
});
```
...
```razor
<Broute Path="/item/{id:int}" RemountOnParameterChange="false">
...
- Rebuilding a route rebuilds everything nested inside it: a child's content lives in the subtree
  ...
  page - the cost of rebuilding on those routes.
...
- **`OnRenavigated`** - a navigation re-committed this route while the *same instance* stayed
  visible (a query-only change, moving between the route's descendants, or `/item/1 → /item/2` on
  a route that [re-binds on a parameter change](#rebuild-or-re-bind-on-a-parameter-change)): the
  "user arrived here again" moment that `OnInitialized` misses on instance reuse. Under the default
  rebuild a route parameter change never raises it - the content is disposed, so it sees a
  `Disposing` deactivation plus a first activation instead. On a per-parameter keep-alive route (`KeepAliveMax` > 1) a parameter change
...
- **`OnRenavigating`** - a pending navigation keeps this route matched and its instance alive (a
  query-only change, moving between its descendants, or a route parameter change on a route that
  [re-binds](#rebuild-or-re-bind-on-a-parameter-change)). This is the case `LeaveGuard`
  deliberately never fires for - without it, a dirty edit form on `/item/1` of a re-binding route
  couldn't veto going to `/item/2`. Under the default rebuild a route parameter change disposes the
  content instead, so it votes through `OnDeactivating` (reason `Disposing`) and its `LeaveGuard`
  runs, exactly as if the route were left.
