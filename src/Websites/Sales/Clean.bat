...
powershell -Command "[string]$trackedFiles = git ls-files; Get-ChildItem -Hidden -Include *.css,*.min.css,*.js,*.min.js,*.map -Recurse | ForEach-Object { if ($trackedFiles -NotMatch $_.Name) { Remove-Item -Recurse -Path $_ -Confirm:$false -Force }}"

:: Runs the dotnet clean command for each .csproj file.
powershell -Command "Get-ChildItem -Hidden -Include *.csproj -Recurse | ForEach-Object { dotnet clean $_.FullName }"

:: Deletes the specified files and folders.
powershell -Command "Get-ChildItem -Hidden -Include *.csproj.user,Resources.designer.cs,bin,obj,node_modules,Packages,TestResults,AppPackages,.meteor -Recurse | ForEach-Object { Remove-Item -Recurse -Path $_ -Confirm:$false -Force }"
