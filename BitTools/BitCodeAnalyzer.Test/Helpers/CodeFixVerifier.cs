...
namespace BitCodeAnalyzer.Test.Helpers
{
    public abstract class CodeFixVerifier : DiagnosticVerifier
    {
        ...
        {
            Document document = await CreateDocument(oldSource, language);
            Diagnostic[] analyzerDiagnostics = await GetSortedDiagnosticsFromDocuments(analyzer, new[] { document });
            IEnumerable<Diagnostic> compilerDiagnostics = await GetCompilerDiagnostics(document);
            ...
            for (int i = 0; i < attempts; ++i)
            {
                ...
                Diagnostic[] diagnostics = compilerDiagnostics.ToArray();
                IEnumerable<Diagnostic> newCompilerDiagnostics = GetNewDiagnostics(diagnostics, await GetCompilerDiagnostics(document));
                ...
                if (!allowNewCompilerDiagnostics && newCompilerDiagnostics.Any())
                {
                    ...
                    newCompilerDiagnostics = GetNewDiagnostics(diagnostics, await GetCompilerDiagnostics(document));
